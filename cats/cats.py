...
def choose(paragraphs, select, k):
    ...
    for i in range(0, len(paragraphs)):
        if select(paragraphs[i]) is True:
...
def accuracy(typed, reference):
    ...
    # the accuracy is dependent on typed, rather than the reference.
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        key = diff_function(user_word, word, limit)
        if not (key in dic_words):
            dic_words[key] = word

    min_key = min(dic_words.keys())
    if min_key > limit:
        return user_word

    return dic_words[min_key]
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if limit < 0 or not goal:
        return len(start)
    elif not start or start[0] != goal[0]:
...
def pawssible_patches(start, goal, limit):
    ...
    if limit < 0 or min(l_start, l_goal) == 0:
        return max(l_start, l_goal)
...
def time_per_word(times_per_player, words):
    ...
    for j in range(len(times_per_player)):
        ...
        for i in range(len(times_per_player[j]) - 1):
            pl[j].append(abs(times_per_player[j][i] - times_per_player[j][i + 1]))


    return game(words,pl)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    lis = [[] for i in player_indices]

    for i in word_indices:
        min = 0
        for j in player_indices:
            if time(game,j,i) < time(game,min,i):
                min = j
        lis[min].append(word_at(game, i))
