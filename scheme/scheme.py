...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        x = scheme_eval(first, env)
        y = rest.map(lambda y: scheme_eval(y, env))
        return scheme_apply(x, y, env)
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        return Frame(self.env).make_child_frame(self.formals,args)
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        exp = LambdaProcedure(target.rest, expressions.rest, env)
        env.define(target.first, exp)
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        validate_form(binding, 2, 2) #each binding is exactly 2
